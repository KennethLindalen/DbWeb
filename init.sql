...
CREATE TABLE IF NOT EXISTS `Idrett` (
  `Idrettskode` INT NOT NULL AUTO_INCREMENT,
  `Navn` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`Idrettskode`))
ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS `Anlegg` (
  `Anleggskode` INT NOT NULL,
  `Idrettskode` INT NOT NULL,
  `Navn` VARCHAR(45) NOT NULL,
  `Åpningstid` INT NOT NULL,
  `Stengetid` INT NOT NULL,
  `Timepris` DECIMAL NOT NULL,
  PRIMARY KEY (`Anleggskode`),
  INDEX `fk_Anlegg_Idrett1_idx` (`Idrettskode` ASC),
  CONSTRAINT `fk_Anlegg_Idrett1`
    FOREIGN KEY (`Idrettskode`)
    REFERENCES `Idrett` (`Idrettskode`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `Reservasjon` (
  ...
  PRIMARY KEY (`Medlemsnummer`, `Anleggskode`, `Dato`, `Klokkeslett`),
  INDEX `fk_Medlem_has_Anlegg_Anlegg1_idx` (`Anleggskode` ASC),
  INDEX `fk_Medlem_has_Anlegg_Medlem1_idx` (`Medlemsnummer` ASC),
  CONSTRAINT `fk_Medlem_has_Anlegg_Medlem1`
    FOREIGN KEY (`Medlemsnummer`)
    REFERENCES `Medlem` (`Medlemsnummer`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_Medlem_has_Anlegg_Anlegg1`
    FOREIGN KEY (`Anleggskode`)
    REFERENCES `Anlegg` (`Anleggskode`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;
